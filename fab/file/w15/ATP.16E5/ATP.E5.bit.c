//
// ATP.E5.bit.c
//    ATP E5 bit echo ring oscillator
//
// Neil Gershenfeld
// 12/20/15
//
...
//
// ATP_process_bit_out
//    nothing to do after bit is sent
//
...
//
// ATP_process_bit_in
//    move bit from in to out
//
#define ATP_process_bit_in(ports,id) {\
   ATP_push_bit_out(ports,id,ports[id].in.bit.value);\
   ATP_pop_bit_in(ports,id);\
   }
//
// main
//
int main(void) {
   ...
   */
   ...
   // initial conditions
   //
   ATP_push_bit_out(ports,0,1);
   //
   // event loop
   //
   while (1) {
      //ATP_output(ports,0,ATP_x);
      //ATP_output(ports,0,ATP_1);
      //ATP_output(ports,0,ATP_x);
      //ATP_output(ports,0,ATP_o);
      ATP_scan(ports,0);
      ATP_scan(ports,1);
      ATP_scan(ports,2);
      ATP_scan(ports,3);
